...
/**
 * Expands the size of the bitmap by the specified number of bits.
 * Newly added bits are initialized to 0.
 *
 * 비트맵의 크기를 주어진 비트 수만큼 확장하는 함수.
 * 새로 확장된 비트들은 0으로 초기화.
 */
struct bitmap *bitmap_expand (struct bitmap *bitmap, size_t size) {
  ASSERT (bitmap != NULL);
    
  size += bitmap->bit_cnt;
    
  if (size <= bitmap->bit_cnt)
      return bitmap;
        
  elem_type *new_bits = realloc(bitmap->bits, byte_cnt(size));
  if (new_bits == NULL)
      return NULL;
  ...
  // 새로 할당된 비트들을 0으로 초기화
  size_t i;
  for (i = elem_cnt(bitmap->bit_cnt); i < elem_cnt(size); i++)
      bitmap->bits[i] = 0;
...
/**
 * Prints all bits in a bitmap as 0s and 1s.
 * Bits are printed in order from left to right.
 *
 * 비트맵의 모든 비트를 0과 1로 출력하는 함수.
 * 비트는 왼쪽부터 오른쪽으로 순서대로 출력됨.
 */
void print_bitmap(struct bitmap *bitmap) {
    size_t i;
